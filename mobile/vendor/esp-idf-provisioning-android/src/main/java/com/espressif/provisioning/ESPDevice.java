...
public class ESPDevice {
    ...
    public ESPDevice(Context context, ESPConstants.TransportType transportType, ESPConstants.SecurityType securityType) {
        ...
        switch (transportType) {
            ...
            case TRANSPORT_BLE:
                transport = new BLETransport(context, this);
                break;
    ...
    public void connectBLEDevice(BluetoothDevice bluetoothDevice, String primaryServiceUuid) {
        ...
        if (transport instanceof BLETransport) {
            ...
            EventBus.getDefault().post(new DeviceConnectionEvent(ESPConstants.EVENT_DEVICE_CONNECTION_FAILED, ESPDevice.this));
        }
    }
    ...
    public void connectWiFiDevice(String ssid, String password) {
        ...
        // Local security patch: never write Wi-Fi credentials to Logcat.
    ...
    public void initSession(final ResponseListener listener) {
        ...
        try {
            ...
            // Local security patch: never write the registration secret to Logcat.
    ...
    private void processGetSSIDs(byte[] responseData) {
        ...
        try {
            ...
            for (int i = 0; i < response.getEntriesCount(); i++) {
                ...
                // SSID is protobuf bytes. Do not silently replace invalid UTF-8,
                // because the displayed value could then identify a different network.
                if (!response.getEntries(i).getSsid().isValidUtf8()) {
                    continue;
                }
        ...
        public void run() {
            ...
            transport.sendConfigData(ESPConstants.HANDLER_PROTO_VER, tempData.getBytes(), new ResponseListener() {
                ...
                public void onSuccess(byte[] returnData) {
                    ...
                    EventBus.getDefault().post(new DeviceConnectionEvent(ESPConstants.EVENT_DEVICE_CONNECTED, ESPDevice.this));
                }
