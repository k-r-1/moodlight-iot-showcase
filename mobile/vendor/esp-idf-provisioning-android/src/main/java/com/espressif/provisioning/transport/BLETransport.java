...
public class BLETransport implements Transport {
     ...
     * @param context
     */
    public BLETransport(Context context, Object eventSource) {
    ...
    private BluetoothGattCallback gattCallback = new BluetoothGattCallback() {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            ...
            if (status == BluetoothGatt.GATT_FAILURE) {
                EventBus.getDefault().post(new DeviceConnectionEvent(ESPConstants.EVENT_DEVICE_CONNECTION_FAILED, eventSource));
                return;
            } else if (status == 133) {
                EventBus.getDefault().post(new DeviceConnectionEvent(ESPConstants.EVENT_DEVICE_CONNECTION_FAILED, eventSource));
                return;
            ...
            if (newState == BluetoothProfile.STATE_CONNECTED) {
                ...
                EventBus.getDefault().post(new DeviceConnectionEvent(ESPConstants.EVENT_DEVICE_DISCONNECTED, eventSource));
            }
        }
        ...
        public void onCharacteristicRead(BluetoothGatt gatt,
            ...
            if (uuidMap.get((ESPConstants.HANDLER_PROTO_VER)).equals(characteristic.getUuid().toString())) {
                ...
                EventBus.getDefault().post(new DeviceConnectionEvent(ESPConstants.EVENT_DEVICE_CONNECTED, eventSource));
            }
