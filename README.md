...
| KISA 기준·적용 범위 | 현재 프로젝트 구현 | 사전 판정 | 준비할 증적 |
|---|---|---|---|
| **§1.3 기기 인증**<br>기기 Basic·Standard | 기기별 QR 등록 비밀로 BLE 상대를 확인하고, 등록 후에는 기기별 X.509 인증서로 AWS IoT에 연결 | **활용 가능**<br>정상 등록은 확인했으나 거부 경로는 미검증 | 다른 기기·잘못된 QR·재사용 시도의 거부 결과, 등록 비밀 생성·주입·폐기 절차 |
| **§2.1 전송정보 보호**<br>모든 기기 등급·앱 Basic | BLE 등록 구간은 Security 2의 AES-GCM, 운영 구간은 MQTT TLS 사용 | **부분 입증**<br>정상 통신은 확인했으나 변조·재전송 차단은 미검증 | 프로토콜·버전·암호 설정·키 처리 명세, 변조·재전송 시험 결과 |
| **§2.2 저장정보 보호**<br>모든 기기 등급·앱 Basic | Wi-Fi 자격정보와 Fleet 기기 인증서·개인키를 NVS/Flash에 저장 | **보완 필요**<br>현재 저장 암호화 미적용, 동등 보호 수단도 미입증 | NVS 암호화·안전한 저장영역·접근통제 중 선택한 보호 설계와 실기기 확인 결과 |
| **§3.1 안전한 암호 알고리즘**<br>모든 기기 등급·앱 Basic | AES-GCM과 TLS 기반 구현 사용 | **명세 필요**<br>알고리즘 이름만으로 전체 기준 충족을 판정할 수 없음 | 실제 알고리즘·키 길이·라이브러리 버전·설정과 시험 결과 |
| **§4.4 알려진 취약점 조치**<br>모든 기기 등급·앱 Basic | Security 2 관련 공개 보안 권고를 확인해 ESP-IDF v5.5.5로 갱신하고 재빌드·실기기 재시험 | **활용 가능**<br>취약점 관리 증적의 한 사례 | 사용 버전 목록, 영향 분석, 조치 커밋·빌드 식별값, 조치 전후 시험 결과 |

공식 근거: [KISA 제도 안내](https://www.kisa.or.kr/1050608) · [상세해설서 배포 게시물](https://www.ksecurity.or.kr/user/bbs/kisis/92/539/bbsDataView/19547.do) · [PDF 원문](https://www.ksecurity.or.kr/common/proc/kisis/bbs/92/fileDownLoad/3625.do). 적용표는 12~14쪽, 준수명세서와 증빙자료 요구는 17쪽, 위 기준은 각각 28~30·37~42·52~54·71~72쪽에 근거한다. ESP-IDF 갱신 근거는 [Espressif 공식 보안 권고 GHSA-9r76-858f-v6jh](https://github.com/espressif/esp-idf/security/advisories/GHSA-9r76-858f-v6jh)다.

※ **ESP-IDF Security 2와 KISA 해설서의 Bluetooth 보안모드 2는 서로 다른 개념**이다. Security 2를 사용한다는 이유만으로 Bluetooth 링크 계층이나 인증 기준 전체를 충족한다고 판정하지 않는다. 최종 적합 여부는 목표 등급의 전체 기준과 인증시험 결과로 판단한다.
