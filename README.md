...
> **공유용 코드 스냅샷** — 코드·화면·공식 근거 요약을 포함하며, 내부 문서(`docs/`)·진행 기록·실제 인증정보·기존 커밋 이력은 제외했다. 아래 검증 결과는 원본 프로젝트의 2026-09-08 시험 기록 기준이며, 이 공유본을 별도 환경에 배포·재시험한 결과는 아니다. 실행·배포에는 별도 환경 설정이 필요하다.
