...
이 디렉터리는 기본값으로 배포가 꺼지는 Terraform 구성이다. 격리된 개인 dev 접두사에 DynamoDB 7개, Cognito User Pool·HTTP API, API·Ingest·Scheduled Lambda, IoT/Fleet 기반, `state`·`tele`·`evt` Rule, Scheduler, Device Registry와 Fleet Guard를 단계별로 배포했다. 기능 활성값은 Git에서 제외한 회사 Mac의 `terraform.tfvars`에만 둔다.
...
Device GSI의 문서상 정렬 키 값은 `POOL#{poolId}#DEVICE#{deviceId}`이지만 물리 필드명은 정해져 있지 않았다. 이 뼈대와 배포된 테이블은 최소 구현 이름으로 `tenantPoolKey`를 사용한다. 백엔드 handler가 이 값을 생성·조회하며 개인 격리 dev의 실제 기기 목록에서 통합 검증했다.

IoT/Fleet는 회사 208의 실제 Terraform을 읽기 전용으로 대조해 격리 dev에 배포했다. 기본값은 꺼져 있고 `onboarding-juwon-*`·`dev` 외에는 guard가 거부한다. 전용 Claim 인증서는 Terraform 밖에서 생성해 정책만 연결했으며 등록 시험 뒤 다시 `INACTIVE`로 닫았다. 실제 Registry seed·Fleet 등록·Runtime 전환·Topic Rule·Ingest·제어·예약을 한 개인 기기로 검증했다. 남은 것은 부정 시나리오, 소유권 해제 후 재등록과 자동 생성된 Fleet 리소스의 정리 검증이다. `iot:RegisterThing`의 정확한 `Resource = "*"` 한 건은 AWS가 리소스 유형을 제공하지 않는 예외이며, 템플릿 전용 신뢰 조건과 나머지 작업의 접두사 제한으로 보완했다.
...
| 변수 | 확인 대상 |
|---|---|
| `project_token` | 회사가 승인한 공식 project token. `moodlight`는 아직 제안값 |
| `environment` | 회사 환경 토큰. 현재 의도는 `dev` |
| `aws_region` | 회사가 승인한 단일 AWS Region |
| `aws_account_id` | 배포를 허용한 12자리 계정 ID |
| `owner_tag` | 비용·운영 소유자를 나타내는 회사 태그 값 |
| `cognito_callback_urls` | Expo 딥링크/배포 앱의 정확한 callback URL |
| `cognito_logout_urls` | 배포 앱의 정확한 logout URL |
| `additional_tags` | 회사 필수 태그. 공통 태그 이름은 덮어쓸 수 없음 |
...
현재 `infra/terraform.tfstate`는 배포된 DynamoDB·API/Cognito/Lambda·IoT/Fleet 리소스를 추적한다. 이 state에서 배포 플래그를 끄면 기존 리소스를 없애는 계획이 생길 수 있으므로 **기본값이나 `terraform.tfvars.example`로 plan하지 않는다.** 이 회사 Mac의 Git 제외 `terraform.tfvars`에는 기존 배포 입력을 복원해 두었다. plan 전 아래 세 가지를 모두 확인한다.

1. 현재 디렉터리에 기존 `terraform.tfstate`와 그 state에 맞는 `terraform.tfvars`가 함께 있음
2. 계정은 명시한 회사 계정, 리전은 `ap-northeast-2`, project token은 `onboarding-juwon-*`
...
state와 plan 파일에는 계정 정보가 들어갈 수 있어 Git·외부에 업로드하지 않는다. Remote State가 아직 없으므로 Git clone만 받은 다른 컴퓨터에서는 이 stack을 plan·apply하지 않는다. 다른 컴퓨터에서 관리해야 한다면 회사가 승인한 state 인계나 Remote State를 먼저 구성한다.

## 이후 변경·배포 절차

추가 리소스를 만들거나 배포 구성을 바꿀 때는 회사 원본과 설계를 다시 대조하고 담당자 승인을 받은 뒤 아래 두 값을 함께 확인한다.
...
출고 등록 보호 기능은 `enable_fleet_registration_guard=false`가 기본이다. 현재 회사 Mac의 Git 제외 설정에서는 명시적으로 활성화해 Device Registry와 Fleet 사전 검증 Hook을 격리 dev에 배포했다. 다음 순서로 별도 계획을 검토하고 활성화했다.
...
2026-09-04 회사 Mac에서 Terraform v1.15.8과 공식 AWS Provider v6.63.0을 사용했다. 명시적 승인 후 격리 dev 접두사에 DynamoDB 7개, API/Cognito/Lambda 15개, IoT/Fleet 8개를 각각 Create-only 계획으로 배포했다. 세 적용 모두 기존 리소스 변경·삭제는 0건이었고 마지막 전체 plan은 0/0/0이다. 앱 설정과 일치하는 `openiot-moodlight://auth/callback`·`openiot-moodlight://auth/logout`과 로컬 CORS를 사용했다. 현재 state는 로컬로 보존하며 Git·외부에 업로드하지 않고, Remote State는 아직 구성하지 않았다.
...
모바일 PKCE callback은 현재 배포 기록과 같은 `openiot-moodlight://auth/callback`이다. 회사에서는 `terraform output -json api_slice`의 `api_endpoint`, `cognito_app_client`, `cognito_domain`을 로컬 모바일 환경 변수에 옮긴다. domain은 `https://{cognito_domain}.auth.{aws_region}.amazoncognito.com` 형태로 조합한다.

EventBridge Scheduler와 Scheduled Lambda를 개인 격리 dev에 배포했고, 앱에서 만든 예약이 설정한 분 안에 실행되어 외장 RGB와 `state`가 바뀌는 것을 확인했다. Scheduler는 초 단위 정시 실행을 보장하지 않으므로 앱에 분 단위 정밀도를 안내하고, 90초를 넘긴 전달은 `STALE_DELIVERY`로 건너뛴다.
